...
from seguimiento.serializers.seguimiento_cultivos import ActividadesCultivoSerializer, \
	InsumoSerializer, InsumoCultivoSerializer, CrecimientoCultivoSerializer, PlagaSerializer, CosechaCultivoSerializer, PlagasCultivoSerializer

from seguimiento.models import ActividadesCultivo, Insumo, InsumoCultivo, Plaga, CosechaCultivo, CrecimientoCultivo, \
	PlagasCultivo
...
class ActividadesViewSet(MultipleModelAPIViewSet):
	queryList = [
		(ActividadesCultivo.objects.all(), ActividadesCultivoSerializer),
		(InsumoCultivo.objects.all(), InsumoCultivoSerializer),
		(CrecimientoCultivo.objects.all(), CrecimientoCultivoSerializer),
		(PlagasCultivo.objects.all(), PlagasCultivoSerializer),
	]
	flat = True
	pagination_class = StandardResultsSetPagination
	# filter_backends = (OrderingFilter, SearchFilter)
